...
2. Enable "Allow connections from network clients" under preferences in XQuartz
   #+begin_src bash
   open -a XQuartz
   #set preference to allow connection from network clients, via the xquartz preference panel
   #+end_src
3. Restart XQuartz
